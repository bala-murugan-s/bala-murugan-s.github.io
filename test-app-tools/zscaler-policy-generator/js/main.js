...
import { bindProduct, bindTenant, bindTypes, bindBuilders, goToStep, nextStep, prevStep, renderSummary, updateSidebar, initTabs, toggleTheme, collectAll } from './ui.js';
...
// Actions
document.getElementById('btnGenerate').addEventListener('click',()=>{ collectAll(); generateAll(); renderSummary(); updateSidebar(); drawRulesChart(); });
document.getElementById('btnViewCode').addEventListener('click',()=>{ if(!Object.keys(files).length) { collectAll(); generateAll(); } document.getElementById('codeModal').showModal(); });
